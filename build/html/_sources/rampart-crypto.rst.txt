...
* ``return_option`` controls the type of the returned value. It may be
  either a :green:`Boolean` or an :green:`Object`:
    ...
    * If omitted or :green:`false`, returns a hex-encoded :green:`String`
      (the default).
    * If :green:`true`, returns a raw :green:`Buffer` (a duktape
      ``Uint8Array``-style plain buffer). This is the historical
      "raw" form and is preserved unchanged.
...
Where:
    ...
    * ``return_option`` controls the type of the returned value. It
      may be either a :green:`Boolean` or an :green:`Object`:
        ...
        * If :green:`true`, returns a raw :green:`Buffer` (a duktape
          ``Uint8Array``-style plain buffer). Preserved unchanged for
          backward compatibility.
        * If an options :green:`Object` of the form
          ``{returnType: "hex" | "uint8array" | "buffer"}``, selects
          the corresponding return shape. ``"buffer"`` yields a
          node-style :green:`Buffer` whose prototype includes
          ``toString('hex'|'base64'|...)``, ``indexOf``, etc.
...
Return Value:
    A :green:`Uint8Array`, :green:`Buffer`, or :green:`String`
    depending on ``return_option``. Default is :green:`Uint8Array` of
    ``nBytes`` bytes.
